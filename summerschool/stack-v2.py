...
with model:
    ...
    actions = spa.Actions(
        'dot(prompt, PUSH) --> push = stack*INDEX+state',
        'dot(prompt, SWAP) --> stack = push',
        'dot(prompt, TOP)  --> clean = stack',
        'dot(prompt, POP)  --> push = (stack-top)*~INDEX')
    ...
    def set_prompt(t):
        ...
        if 0.15 < t < 0.25: return 'SWAP'
        if 0.25 < t < 0.35: return 'PUSH'
        if 0.35 < t < 0.45: return 'SWAP'
        if 0.45 < t < 0.55: return 'PUSH'
        if 0.55 < t < 0.65: return 'SWAP'
        if 0.65 < t < 0.75: return 'TOP'
        if 0.75 < t < 0.85: return 'POP'
        if 0.85 < t < 0.95: return 'SWAP'
        if 0.95 < t < 1.05: return 'TOP'
        if 1.05 < t < 1.15: return 'POP'
        if 1.15 < t < 1.25: return 'SWAP'
        if 1.25 < t < 1.35: return 'TOP'
        if 1.35 < t < 1.45: return 'POP'
        if 1.45 < t < 1.55: return 'SWAP'
        else: return '0'
    ...
    push_probe = nengo.Probe(model.push.state.output, synapse=0.01)
...
p5.legend(model.get_output_vocab('push').keys, fontsize='x-small')
p5.set_ylabel('Push')
